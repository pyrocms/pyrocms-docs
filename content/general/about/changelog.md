...
* Added order-by attribute to {{ files:listing }} to allow sorting list
...
* Fixed a bug with the {{ pages:chunk }} tag when used with markdown
...
* Fixed #1386. Support for less/sass in the {{ theme:css }} tag
...
* Added a tag: {{ helper:config item="default_language" }}
...
* Change SITE_URL to {{ site:url }} when inserting files and images into ckeditor
...
* {{ pages:children }} now handles page chunks properly
