...
* {{ docs:id_link title="Displaying Your Streams Data" }}
...
In Streams, each "list" of data is known as a "stream". So, a list of bands would be a stream. A list of live shows would be a stream. Streams gives you the tools to create and manage those streams on the back end, and then gives you the tools to display that data on your website easily via PyroCMS page templates.
...
You’ll notice that fields are not specifically linked to their stream right away. This is because Streams is built to allow you to share fields between streams. Instead of having to create a "Name" field over and over again, you can create one and assign it to more and one field.
...
You can also choose some extra settings for this instance of your field. You can tell Streams to make sure its value is unique within the stream, that it is required, and give notes that will appear on the edit and add forms. The following field are available.
...
Streams comes with two types of ordering built-in: **manual** and **automatic**. These two ordering modes affect how information is ordered on the back end when you look through it, as well as how it is ordered when you loop through it in your layouts.
...
<div class="tip"><strong>Note:</strong> You can always break the default sort order in Streams with the order_by and sort parameters in the cycle plugin.</div>
...
Streams comes with two extra permission settings:
...
## Displaying Your Streams Data

Once you have your data structures set up, you'll want to display it in some form in your page layouts and templates. To do that, you can use the {{ link title="Streams plugin" uri="plugins/streams" }}.
